	...
	float heightScale)
{
	// load heightmap
	Bitmap bitmap;
	if (bitmap.loadBMP(bmpFilename.c_str()) == false)
	{
	...
	_tex = NULL;
	_device = device;
	_numVertsPerRow = bitmap.width; // x, width
	_numVertsPerCol = bitmap.height; // z, depth
	...
	LPBYTE map = (LPBYTE)bitmap.data;
	for (int i = 0; i < _numVertices; i++)
		_heightmap[i] = map[3 * i];
...
void Terrain::SetBoundM(int w, int h) {
	_width = w;
	_depth = h;
}
bool Terrain::computeVertices()
{
	...
	float startX = -_width / 2.f;
	float startZ = _depth / 2.f;

	// coordinates to end generating vertices at
	float endX = _width / 2.f;
	float endZ = -_depth / 2.f;
...
float Terrain::computeShade(int cellRow, int cellCol, D3DXVECTOR3* directionToLight)
{
	...
	D3DXVECTOR3 u((float)_cellSpacing, (float)heightB - heightA, 0.0f);
...
{
	if (x < _width*(-0.5f) || x > _width*0.5f || z > _depth*0.5f || z < _depth*(-0.5f))
		return 0;
